...
from light_tts.common.infer_utils import init_req_to_token_indexes
from light_tts.common.build_utils import repair_config
from light_tts.common.basemodel.cuda_graph import CudaGraph
from light_tts.common.basemodel.triton_kernel.copy_kv_index_to_req import copy_kv_index_to_req
...
class TpPartBaseModel:
    ...
    def forward(
            ...
            sample_func=None,
            is_prefill=True, **kwargs):
        if is_prefill:
            return self._prefill(batch_size, total_token_num, max_len_in_batch, input_ids, b_req_idx, b_start_loc, b_seq_len, temperature, sample_func, b_ready_cache_len)
        else:
            return self._decode(batch_size, total_token_num, max_len_in_batch, input_ids, b_req_idx, b_start_loc, b_seq_len, temperature, sample_func)

    
    def _prefill(self, batch_size, total_token_num, max_len_in_batch, input_ids, b_req_idx, b_start_loc, b_seq_len, temperature, sample_func, b_ready_cache_len):
        ...
        infer_state.temperature = temperature
        ...
        infer_state.kv_buffer = torch.empty((input_ids.shape[0], self.tp_k_head_num_ + self.tp_v_head_num_, self.head_dim_), dtype=torch.float16, device="cuda")
        
        init_req_to_token_indexes(
            self.req_manager.req_to_token_indexs, b_req_idx, b_seq_len,
            infer_state.b_ready_cache_len, max_len_in_batch, infer_state.mem_index
        )

        infer_state.init_some_extra_state(self, input_ids)
        predict_logics = self._context_forward(input_ids, infer_state, sample_func)
        return predict_logics
    
    def _decode(self, batch_size, total_token_num, max_len_in_batch, input_ids, b_req_idx, b_start_loc, b_seq_len, temperature, sample_func):
        infer_state = self.infer_state_class()
    ...
    @final
    def _context_forward(self, input_ids, infer_state: InferStateInfo, sample_func):
