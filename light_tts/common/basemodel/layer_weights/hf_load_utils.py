...
def load_func(file_, use_safetensors=False, pre_post_layer=None, transformer_layer_list=None, weight_dir=None, remove_prefix=None):
    ...
    if use_safetensors:
        ...
        if remove_prefix is None:
            ...
            weights = {k.replace(remove_prefix, ''): weights.get_tensor(k) for k in weights.keys()}
    else:
...
def load_hf_weights(data_type, weight_dir, pre_post_layer=None, transformer_layer_list=None, weight_dict=None, remove_prefix=None):
    ...
    if weight_dict:
        if remove_prefix is not None:
            weight_dict = {k.replace(remove_prefix, ''): weight_dict[k] for k in weight_dict.keys()}
        if pre_post_layer is not None:
    ...
    for file in candidate_files:
        load_func(file, use_safetensors=use_safetensors, pre_post_layer=pre_post_layer, transformer_layer_list=transformer_layer_list, weight_dir=weight_dir, remove_prefix=remove_prefix)
    return
