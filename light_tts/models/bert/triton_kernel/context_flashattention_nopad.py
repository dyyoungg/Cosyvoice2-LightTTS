...
def _fwd_kernel(
    ...
    cur_batch_bid_seq_len = tl.load(B_Seqlen + cur_batch)
    ...
    for start_n in range(0, block_mask * cur_batch_seq_len, BLOCK_N):
        ...
        qk = tl.where((offs_m[:, None] < cur_batch_bid_seq_len) & ((start_n + offs_n[None, :]) >= cur_batch_bid_seq_len), -10000000.0, qk)
        qk = tl.where((offs_m[:, None] >= cur_batch_bid_seq_len) & ((start_n + offs_n[None, :]) > offs_m[:, None]), -10000000.0, qk), 
