...
def test():
    ...
    Z, H, N_CTX, D_HEAD = 1, 16, 2048, 128
    dtype = torch.float16
    ...
    q_last = q[-1:, :, :]
    o_last = torch.empty((1, H, D_HEAD), dtype=dtype, device="cuda").normal_(mean=0.3, std=0.2)
    b_prompt_cache_len[0] = N_CTX - 1
    context_attention_fwd(
        q_last, k, v, o_last, b_req_idx, b_start_loc, b_seq_len, b_prompt_cache_len, max_input_len, req_to_token_indexs
    )
    torch.cuda.synchronize()
    b = time.time()
