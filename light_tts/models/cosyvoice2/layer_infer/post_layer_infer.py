...
class CosyVoice2PostLayerInfer(LlamaPostLayerInfer):
    ...
    def _norm(self, input, infer_state, layer_weight: CosyVoice2PreAndPostLayerWeight) -> torch.Tensor:
    ...
    def token_forward(
        ...
        layer_weight: CosyVoice2PreAndPostLayerWeight,
        return_logics=False,
    ):
        ...
        # logic_batch = torch.addmm(layer_weight.lm_head_weight_, last_input)
        logic_batch = torch.addmm(
            layer_weight.llm_decoder_bias_, layer_weight.llm_decoder_weight_, last_input, beta=1.0, alpha=1.0
        )
