...
from light_tts.models.sovits_gpt.module import attentions
from torch.nn import Conv1d, ConvTranspose1d, AvgPool1d, Conv2d
from torch.nn.utils import weight_norm, remove_weight_norm, spectral_norm
from light_tts.models.sovits_gpt.module.commons import init_weights, get_padding
from light_tts.models.sovits_gpt.module.mrte_model import MRTE
from light_tts.models.sovits_gpt.utils.text.symbols2 import symbols
...
class ResidualCouplingBlock(nn.Module):
    def __init__(
        ...
        assert len(self.flows) == 8

    def forward(self, x, x_mask, g):
        x = self.flows[7](x)
        x = self.flows[6](x, x_mask, g=g)
        x = self.flows[5](x)
        x = self.flows[4](x, x_mask, g=g)
        x = self.flows[3](x)
        x = self.flows[2](x, x_mask, g=g)
        x = self.flows[1](x)
        x = self.flows[0](x, x_mask, g=g)
        return x
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        assert self.num_kernels == 3
        assert self.num_upsamples == 5
    ...
    def forward(self, x, g):
        ...
        xs = xs + self.resblocks[1](x)
        xs = xs + self.resblocks[2](x)
        x = xs / self.num_kernels

        x = F.leaky_relu(x, modules.LRELU_SLOPE)
        x = self.ups[1](x)
        xs = self.resblocks[3](x)
        xs = xs + self.resblocks[4](x)
        xs = xs + self.resblocks[5](x)
        x = xs / self.num_kernels

        x = F.leaky_relu(x, modules.LRELU_SLOPE)
        x = self.ups[2](x)
        xs = self.resblocks[6](x)
        xs = xs + self.resblocks[7](x)
        xs = xs + self.resblocks[8](x)
        x = xs / self.num_kernels

        x = F.leaky_relu(x, modules.LRELU_SLOPE)
        x = self.ups[3](x)
        xs = self.resblocks[9](x)
        xs = xs + self.resblocks[10](x)
        xs = xs + self.resblocks[11](x)
        x = xs / self.num_kernels

        x = F.leaky_relu(x, modules.LRELU_SLOPE)
        x = self.ups[4](x)
        xs = self.resblocks[12](x)
        xs = xs + self.resblocks[13](x)
        xs = xs + self.resblocks[14](x)
        x = xs / self.num_kernels
